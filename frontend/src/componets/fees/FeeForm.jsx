...
const FeeForm = ({ open, onClose, onSubmit, editData }) => {
  const theme = useTheme();
  const isMobile = useMediaQuery(theme.breakpoints.down('sm'));
  const [students, setStudents] = useState([]);
  const [formData, setFormData] = useState({
  ...
  useEffect(() => {
    if (editData) {
      setFormData({
        ...
        admissionFees: editData.admissionFees || 0,
        tuitionFees: editData.tuitionFees || 0,
        transportFees: editData.transportFees || 0,
        datePaid: editData.datePaid
          ? new Date(editData.datePaid).toISOString().split('T')[0]
          : new Date().toISOString().split('T')[0],
